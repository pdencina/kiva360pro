...
// GET: Listar evoluciones de un plan
...
// POST: Crear informe de evolución
export async function POST(request: NextRequest) {
  ...
  const { plan_id, periodo, resumen, avances, areas_pendientes, recomendaciones, valoracion, visible_familia } = body

  if (!plan_id || !periodo || !resumen) {
    return NextResponse.json({ error: 'plan_id, periodo y resumen requeridos' }, { status: 400 })
  }

  const { data, error } = await admin.from('evoluciones').insert({
    plan_id,
